...
export default function AdminTopbar({ profile, announcements, onToggleMobileSidebar }) {
  const { user } = useAuth();

  const displayName = profile?.name || user?.name || "System Administrator";
  const displayEmail = profile?.email || user?.email || "admin@fisat.ac.in";
      ...
      <div className="mx-auto flex h-[72px] items-center justify-between px-4 sm:px-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2.5">
            <div className="flex h-8 w-8 items-center justify-center bg-slate-800 text-[12px] font-bold text-white uppercase border border-slate-700">
              {displayName[0] || "A"}
            </div>
